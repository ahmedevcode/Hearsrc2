...
class FavoriteUsers extends StatefulWidget {
  ...
  @override
  State createState() => FavoriteUsersState(currentUserId: currentUserId);
}

class FavoriteUsersState extends State<FavoriteUsers> {
  FavoriteUsersState({Key? key, required this.currentUserId});
  List<String> fav = [];

  final String currentUserId;
  //final String serverToken = 'AAAAEoILIxw:APA91bEH3z_PoP20EdeDa-vBnxiwbzjQuBuNHe5upnN9jEcE1rw8bHOEFWTIjSsSD1mxhs2BCx7jPtscE8ZtRCr7V_iAiYbTnOE19CaFIk_va4rYxyj0JA9XkJFn6g0EoPBfk0RXWaHH';
...
/*@override
  ...
  @override
  void didUpdateWidget(covariant FavoriteUsers oldWidget) {
    //if(fav.length!=oldWidge)
    //print('in update');
                  ...
                  Container(
                    child: StreamBuilder(
                      ...
                      builder: (context, snapshot) {
                        // //print('zzz ${snapshot.data.docs.length}');
                        if (!snapshot.hasData) {
                          return Center(
                              child: Column(
                            children: [
                            ...
                            itemBuilder: (context, index) => FutureBuilder(
                                ...
                                builder: (context, metadata) {
                                  return metadata.hasData
                                      ? fav.contains(snapshot.data!.docs[index]
                                              .data()['id']
                                              .toString())
                                          ? buildItem(
                                              context,
                              ...
                              Row(
                                children: [
                                  Icon(MdiIcons.checkAll,
                                      size: 20,
                                      color: metadata?.data['lastmsgseen']
                                          ? Colors.green
                                          : Colors.grey),
                                  Container(
                                    // margin:EdgeInsets.only(top: 5),
                                    child: Text(
                                      metadata?.data['lastMSG'] ?? '',
